...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 23:11:26 2017
...
def HD_PI_integrated_net(N_HD, N_speed, N_x_axis, N_y_axis, sim_time):
    ...
    Poisson_Left = PoissonGroup(1, rates=3000*Hz,name='poisson_drive_left')   #Poisson Stimuli group for the first
    #change with speed
    #### #left drive pspeedgroup

    Speed_Neuron_left = NeuronGroup(N_speed, eqs_HD,threshold='v>1',reset='v=0',method='exact')

    Left_drive = [ Speed_Neuron_left[(inp):(inp + 1)] for inp in range(N_speed)]

    Poisson_Right= PoissonGroup(1, rates=3000*Hz,name='poisson_drive_right')   #Poisson Stimuli group for the first
    ##### right drive speed group
    Speed_Neuron_right= NeuronGroup(N_speed, eqs_HD,threshold='v>1',reset='v=0',method='exact')

    Right_drive = [ Speed_Neuron_right[(inp):(inp + 1)] for inp in range(N_speed)]
    ...
    Poisson_speed_synapseleft = Synapses(Poisson_Left, Speed_Neuron_left, 'w_psl:1', on_pre='v_post+=w_psl',
                                     name='poisson_speed_SynapseL')
    Poisson_speed_synapseleft.connect()

    Poisson_speed_synapseright = Synapses(Poisson_Right, Speed_Neuron_right, 'w_psr:1', on_pre='v_post+=w_psr',
                                     name='poisson_speed_SynapseR')
    Poisson_speed_synapseright.connect()
    ...
    #go straight
    Go_straight_neuron = NeuronGroup(N_HD ,eqs_HD,threshold='v>1',reset='v=0',method='linear',name='go_Straight_neuron')
